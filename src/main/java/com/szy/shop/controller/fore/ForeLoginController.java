...
public class ForeLoginController extends BaseController {
    ...
    @ApiOperation(value = "数字验证码", notes = "数字验证码")
    @ResponseBody
    @GetMapping(value = "login/code")
    public ApiVerCodeResp getVerCode() {
        // 创建一个数字验证码，宽度140，高度38，验证码字符个数4，干扰线数目20
        LineCaptcha captcha = CaptchaUtil.createLineCaptcha(140, 38, 4, 20);
        String code = captcha.getCode();  // 获取验证码的字符
        return new ApiVerCodeResp(String.valueOf(UUID.randomUUID()), captcha.getImageBase64Data(), code.toLowerCase());
    }
}
